...
faq = {
    ("invoice", "invoices", "invoicing", "bill"): "📄 Send invoices to finance@shook.digital with your PO number.",
    ...
    ("project files", "files", "documents", "assets", "drive"): "📁 Access shared Drive folders per project, or contact ops.",
...
if st.button("Ask"):
    ...
    for keywords, answer in faq.items():
        if any(keyword in user_q_lower for keyword in keywords):
            ...
            break
    if not response:
        response = "🤔 Please email ops@shook.digital for that."
    st.success(response)